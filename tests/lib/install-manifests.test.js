...
function runTests() {
  ...
  if (test('lists install modules from the real project', () => {
    ...
    assert.ok(modules.some(module => module.id === 'unity-cpp'), 'Should include unity-cpp');
  })) passed++; else failed++;

  if (test('lists install components from the real project', () => {
    const components = listInstallComponents();
    assert.ok(components.some(component => component.id === 'lang:cpp'),
      'Should include lang:cpp');
  ...
  if (test('resolves a real project profile with target-specific skips', () => {
    ...
    const plan = resolveInstallPlan({ profileId: 'unity', target: 'cursor', projectRoot });
    ...
    assert.ok(!plan.selectedModuleIds.includes('hooks-runtime') || plan.selectedModuleIds.includes('hooks-runtime'),
      'hooks-runtime may or may not be skipped depending on cursor support');
  ...
  if (test('resolves legacy compatibility selections into manifest module IDs', () => {
    const selection = resolveLegacyCompatibilitySelection({
      target: 'cursor',
      legacyLanguages: ['cpp', 'csharp'],
    });

    assert.deepStrictEqual(selection.legacyLanguages, ['cpp', 'csharp']);
    ...
    assert.ok(selection.moduleIds.includes('unity-cpp'));
  })) passed++; else failed++;

  if (test('resolves cpp legacy compatibility into unity-cpp module', () => {
    const selection = resolveLegacyCompatibilitySelection({
      target: 'cursor',
      legacyLanguages: ['cpp'],
    });
    ...
    assert.ok(selection.moduleIds.includes('rules-core'));
    assert.ok(selection.moduleIds.includes('unity-cpp'),
      'cpp should resolve to unity-cpp module');
  })) passed++; else failed++;

  if (test('resolves csharp legacy compatibility into unity-cpp module', () => {
    const selection = resolveLegacyCompatibilitySelection({
      target: 'cursor',
    ...
    assert.ok(selection.moduleIds.includes('rules-core'));
    assert.ok(selection.moduleIds.includes('unity-cpp'),
      'csharp should resolve to unity-cpp module');
  })) passed++; else failed++;

  if (test('resolves go legacy compatibility into base modules only', () => {
    const selection = resolveLegacyCompatibilitySelection({
      target: 'cursor',
      legacyLanguages: ['go'],
    });
    ...
    assert.ok(selection.moduleIds.includes('rules-core'));
    assert.ok(!selection.moduleIds.includes('unity-cpp'),
      'go should not resolve to unity-cpp module');
  })) passed++; else failed++;
  ...
  if (test('resolves included and excluded user-facing components', () => {
    const plan = resolveInstallPlan({
      ...
      excludeComponentIds: ['capability:agentic'],
      target: 'claude',
    });

    assert.deepStrictEqual(plan.includedComponentIds, ['capability:security']);
    assert.deepStrictEqual(plan.excludedComponentIds, ['capability:agentic']);
    assert.ok(plan.selectedModuleIds.includes('security'), 'Should include modules from selected components');
    assert.ok(!plan.selectedModuleIds.includes('agentic-patterns'), 'Should exclude modules from excluded components');
  })) passed++; else failed++;
  ...
  if (test('fails when a selected component depends on an excluded component module', () => {
    assert.throws(
      () => resolveInstallPlan({
        includeComponentIds: ['capability:security'],
        excludeComponentIds: ['baseline:workflow'],
      }),
      /depends on excluded module workflow-quality/
    );
  })) passed++; else failed++;
